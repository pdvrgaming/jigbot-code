//+------------------------------------------------------------------+
//|                                           Zing_M1_Scalper_EA_v3.mq5 |
//|                                  Copyright 2026, JuguruJinganiya |
//|                                      https://jugurujinganiya.bot |
//+------------------------------------------------------------------+
#property copyright   "Copyright 2026, JuguruJinganiya"
#property link        "https://jugurujinganiya.bot"
#property version     "3.00"
#property description "Zing M1 Scalper v3 - Staggered 3-Order Recovery Grid (Levels 1-10) & Dynamic VWAP Basket"
#property strict
...
//--- Input Parameters ---
input group "=== 1. INITIAL TRADE PARAMETERS ==="
...
input string   InpTradeComment      = "Zing Cloned"; // Order Comment

input group "=== 2. 3-ORDER CLUSTER RECOVERY GRID ==="
...
input group "=== 3. ENTRY STRATEGY FILTER ==="
...
int OnInit()
{
   ...
   PrintFormat("Zing_M1_Scalper_EA v1.10 initialized successfully on %s %s. Magic: %I64u", 
               _Symbol, EnumToString(InpTimeframe), InpMagicNumber);
   PrintFormat("Settings: InitialLot=%.2f, TP=%.2f pts, GridStep=%.2f pts, MaxLevels=%d, BasketTP=%.2f pts",
               InpInitialLot, InpTakeProfitPoints, InpGridStepPoints, InpMaxGridLevels, InpBasketTpPoints);
...
{
   PrintFormat("Zing_M1_Scalper_EA deinitialized. Reason: %d", reason);
}
...
{
   Print(">> Closing all open basket positions...");
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(m_position.SelectByIndex(i))
      {
...
void CheckBasketExit(ENUM_POSITION_TYPE basket_type, double vwap, int count)
{
   ...
   if(basket_type == POSITION_TYPE_BUY)
   {
      ...
      if(bid >= target_price)
      {
         PrintFormat(">> BUY Basket Target Reached: Bid=%.2f >= VWAP+TP=%.2f. Closing all %d positions in profit...", 
   ...
   else if(basket_type == POSITION_TYPE_SELL)
   {
      ...
      if(ask <= target_price)
      {
         PrintFormat(">> SELL Basket Target Reached: Ask=%.2f <= VWAP-TP=%.2f. Closing all %d positions in profit...", 
...
//| Level 1:  3 x 0.01 lot                                           |
//| Level 2:  0.01 + 2 x 0.02 lot                                    |
//| Level N:  0.01 + 2 x (N * 0.01) lot                              |
//| Level 10: 0.01 + 2 x 0.10 lot                                    |
//+------------------------------------------------------------------+
...
void CheckInitialEntry()
{
   ...
   // Candle closed bearish, broke previous low, and extended lower than 2 bars ago
   ...
   // Candle closed bullish, broke previous high, and extended higher than 2 bars ago
...
{
   if(!InpEnableRecovery || m_cluster_pending)
      return;
...
void OnTick()
{
   ...
   // No Sleep() is used, so the EA remains responsive.
   ...
   if(!has_positions)
   {
      ...
      if(m_current_grid_level > 0 || m_initial_price > 0.0 ||
         m_cluster_pending)
      {
         m_current_grid_level = 0;
         m_initial_price = 0.0;
      ...
      // Keep the original candle logic. The only upgrade is that after a
      // basket has just closed, the latest closed candle is checked immediately.
